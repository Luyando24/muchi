  ...
  const fetchUserProfile = async () => {
    try {
      ...
      if (profile) {
        ...
        const names = (profile.full_name || 'System Admin').split(' ');
        setUser({
          firstName: names[0],
      ...
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              {isSidebarOpen ? <XCircle className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
            </Button>
            <div className="flex items-center gap-3">
              ...
              <div className="hidden md:block">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-400">System Administration</p>
              </div>
            </div>
            {/* Breadcrumb */}
                      ...
                      {searchResults.schools.length === 0 && searchResults.users.length === 0 ? (
                        ...
                        <>
                          {searchResults.schools.length > 0 && (
                            <div className="py-2">
                              <h4 className="px-4 py-1 text-xs font-semibold text-muted-foreground uppercase">Schools</h4>
                              {searchResults.schools.map(school => (
                                <div 
                                  key={school.id} 
                                  className="px-4 py-2 hover:bg-muted cursor-pointer flex justify-between items-center"
                                  onClick={() => {
                                    if (setActiveTab) setActiveTab('schools');
                                    setIsSearchOpen(false);
                                  }}
                                >
                                  ...
                                  <Badge variant="outline" className="text-xs">{school.plan}</Badge>
                                </div>
                              ))}
                            </div>
                          )}
                          {searchResults.users.length > 0 && (
                            <div className="py-2">
                              <h4 className="px-4 py-1 text-xs font-semibold text-muted-foreground uppercase">Users</h4>
