...
import { Loader2, CheckCircle2, AlertCircle, Clock, BookOpen, ChevronRight, Trophy, Sparkles, ArrowLeft, Filter } from 'lucide-react';
...
export default function TeacherVerifyGrades() {
  ...
  useEffect(() => {
    // Generate a few years around current year for the dropdown
  ...
  const fetchStatus = async () => {
    try {
      ...
      // Fetch settings to get official exam types and current academic year
      try {
        ...
        if (settings) {
          ...
          if (settings.academic_year) {
             ...
             setSelectedAcademicYear(prev => prev === new Date().getFullYear().toString() ? settings.academic_year : prev);
          }
        }
      } catch (e) {
        console.error("Failed to load school settings for verification page", e);
      }

      const data = await syncFetch('/api/teacher/verify-status', { headers });
      setStatuses(data);
    } catch (error) {
  ...
  const submittedCount = filteredStatuses.filter(s => s.status === 'Submitted' || s.status === 'Published').length;
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-4 md:p-8">
      <div className="space-y-6 max-w-5xl mx-auto">
        <Button variant="ghost" onClick={() => navigate('/teacher-portal')} className="mb-4">
